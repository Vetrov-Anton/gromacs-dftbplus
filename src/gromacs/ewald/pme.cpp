               ...
               real*                          pot)
{
    ...
    {
        GMX_ASSERT(bForQMMM || coordinates.ssize() == atc.numAtoms(),
                   "We expect atc.numAtoms() coordinates");
        GMX_ASSERT(bForQMMM || forces.ssize() >= atc.numAtoms(),
    ...
    for (grid_index = 0; grid_index < max_grid_index; ++grid_index)
    {
        ...
        spread_on_grid(pme, &atc, pmegrid, bFirst || (bForQMMM && !bMMforcesOnly), TRUE,
...
#pragma omp parallel num_threads(pme->nthread) private(thread)
        ...
        if (bForQMMM && stepWork.computePotentials)
        {
            ...
            for (thread = 0; thread < pme->nthread; thread++)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        gather_f_bsplines(pme, grid, TRUE, &atc, &atc.spline[thread],
                                          1.0,
