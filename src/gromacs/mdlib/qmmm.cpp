...
// Set up the topological (1-2, 1-3, 1-4) exclusions of the QM/MM electrostatics.
//
// By default, every MM atom on the short-range list contributes its full point
// charge to the external potential entering the QM Hamiltonian, no matter
// whether it is covalently bound to a QM atom: the exclusions generated by
// grompp only ever act on the classical nonbonded interactions and never
// reach the QM calculation. This routine optionally reproduces the exclusion
// convention of the force field for the QM--MM electrostatics as well.
//
...
void QMMM_rec::init_QMMM_exclusions(const gmx_mtop_t* mtop, const t_forcerec* fr)
{
    ...
    const char* env = getenv("GMX_QMMM_NREXCL");
    qmmmNrexcl      = (env == nullptr) ? 0 : atoi(env);
    ...
    qmmmFudgeQQ = fr->fudgeQQ;
    if ((env = getenv("GMX_QMMM_FUDGEQQ")) != nullptr)
    {
        qmmmFudgeQQ = atof(env);
    }

    // Connectivity of the entire system, in global atom numbering.
    std::vector<std::vector<int>> bonds(mtop->natoms);
    int                          atomOffset = 0;
    for (const gmx_molblock_t& molb : mtop->molblock)
    {
        ...
        for (int mol = 0; mol < molb.nmol; mol++)
        {
            for (int ftype = 0; ftype < F_NRE; ftype++)
            {
                ...
                for (int i = 0; i < il.size(); i += 3) // 2 atoms + 1 function type
                {
                    const int a1 = atomOffset + il.iatoms[i + 1];
                    const int a2 = atomOffset + il.iatoms[i + 2];
    ...
    std::vector<bool> bQM(mtop->natoms, false);
...
// Map the scaling factors of the excluded QM--MM pairs onto the current
...
void QMMM_rec::update_QMMM_exclusion_scaling(int natoms)
{
    ...
    mm_.qmmmScale.assign(static_cast<size_t>(qm_.nrQMatoms) * mm_.nrMMatoms, real(1.0));
    for (int j = 0; j < qm_.nrQMatoms; j++)
    {
        for (const std::pair<int, real>& exc : mmScaleExc[j])
        {
            ...
            mm_.qmmmScale[static_cast<size_t>(j) * mm_.nrMMatoms + k] = exc.second;
        }
    }
}
